...
func _physics_process(delta):
	...
	if is_on_floor():
		...
		elif is_loading_jump && (Input.is_action_just_released("jump") || (Time.get_unix_time_from_system() - jump_load_start) > .2):
			is_loading_jump = false
			var timer_proportion = .5 + 5./2.*clamp(Time.get_unix_time_from_system() - jump_load_start, 0, .2)
	...
	if Input.is_action_just_pressed("move_left"):
		...
		if ts - previous_dash > dash_cooldown:
			if ts - previous_right < dash_window:
				...
				hz_velocity = 3000
			previous_right = ts
	hz_velocity = velocity.x - (16*delta*velocity.x) if abs(velocity.x) > abs(hz_velocity) else hz_velocity
