...
@export var net_worth_percent_threshold := 100.0
@export var cooldown := 0.0
@export var minimum_difficulty_factor := 1.0

# Normalized distance to the player
# 0.0 means the player is next to boss
# 1.0 means the player and the boss are in each walls
@export var minimum_distance_x := 0.0
@export var maximum_distance_x := 1.0
@export var prefered_distance_x := 0.3
@export var prefered_distance_tolerance_x := 0.1

# Can be lowered if the attack is more rare
@export var base_weigth := 1.0
...
func calculate_weight(
	...
	var normalized_distance: float = clamp(
		distance_to_player_x / theoretical_max_distance_x, 0.0, 1.0
	)
	if (
		not enabled
		or not is_ready()
